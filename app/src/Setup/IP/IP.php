...
class IP
{
    protected const IP = "172.24.0.254";
    ...
    public function removeIPFromHosts(Config $config) : void
    {
        if (is_null($this->filesystem->getLineInFile("/hosts", self::IP))) {
            throw new \RuntimeException("IP " . self::IP . " not found!");
        }

        $line = self::IP . " " . $config->getHost() . "\n";

        $this->filesystem->replaceStringInFile("/hosts", $line, "");
    }
}