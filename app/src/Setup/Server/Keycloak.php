...
class Keycloak
{
    ...
    public function start(LoggerInterface $logger, Config $config) : void
    {
        $this->filesystem->replaceStringInFile(
            ...
            "%TPL_SERVER_HOSTNAME%",
            $config->getKeycloakHostname()
        );

        $this->filesystem->replaceStringInFile(
            "/usr/local/lib/doil/server/keycloak/conf/keycloak-startup.conf",
            "%TPL_DB_USERNAME%",
            $config->getKeycloakDbUsername()
        );

        $this->filesystem->replaceStringInFile(
            "/usr/local/lib/doil/server/keycloak/conf/keycloak-startup.conf",
            "%TPL_DB_PASSWORD%",
            $config->getKeycloakDbPassword()
        );
        ...
        $this->filesystem->replaceStringInFile(
            ...
            "%TPL_NEW_ADMIN_PASSWORD%",
            $config->getKeycloakNewAdminPassword()
        );

        $this->filesystem->replaceStringInFile(
            "/usr/local/share/doil/stack/states/keycloak/keycloak/init.sls",
            "%TPL_OLD_ADMIN_PASSWORD%",
            $config->getKeycloakOldAdminPassword()
        );
